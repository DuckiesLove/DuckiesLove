(() => {
  const TAG = '[TK rail]';
  if (window.__TK_SCORE_RAIL__) { console.debug(TAG,'already mounted'); return; }
  ...
  css.textContent = `
    ...
    .tk-rail h3{ margin:0 0 10px; font:600 14px/1.2 ui-sans-serif,system-ui; color:#bfefff; text-shadow:0 0 10px rgba(0,224,255,.35) }
    .tk-row{ display:flex; flex-wrap:wrap; gap:10px; align-items:stretch }
    .tk-chip{ display:inline-flex; align-items:center; gap:8px; padding:8px 10px; background:var(--tk-chip-bg); border-radius:var(--tk-chip-br); border:1px solid rgba(255,255,255,.08); box-shadow: inset 0 0 0 1px rgba(255,255,255,.04); white-space:nowrap; font-size:13px }
    .tk-num{ width:22px;height:22px;min-width:22px;display:grid;place-items:center;border-radius:999px;font-weight:700;color:#021016; box-shadow:0 0 0 2px rgba(0,0,0,.35) inset }
    .tk-0 .tk-num{ background:#9bdcff } /* blue joke/skip */
    .tk-1 .tk-num{ background:#ff7474 } /* red   */
    .tk-2 .tk-num{ background:#ffd04d } /* yellow*/
  ...
  console.debug(TAG,'rendered & fitted');

  function fit() {
    const cat = document.getElementById('categoryPanel') || document.querySelector('[aria-label="Categories"]');
    const leftEdge = cat ? cat.getBoundingClientRect().right : 0;
    const available = window.innerWidth - leftEdge - 32; // 16px margin each side
    const desired = 420;
  ...
  function chip(n, txt){
    return `<span class="tk-chip tk-${n}"><span class="tk-num">${n}</span><span>${txt}</span></span>`;
  }
})();
